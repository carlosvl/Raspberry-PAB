#!/usr/bin/env bash
# Upload ESP32 matrix wiring smoke test (768 LEDs on GPIO 16).
set -euo pipefail

ROOT="$(cd "$(dirname "${BASH_SOURCE[0]}")/.." && pwd)"
FQBN="${PAB_ESP32_FQBN:-esp32:esp32:esp32}"
SKETCH="${ROOT}/hardware/esp32/matrix_test"
...
echo "Uploaded ESP32 matrix_test to ${PORT} (re-upload production with upload-esp32-hardware.sh after wiring check)"
